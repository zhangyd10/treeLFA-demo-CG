...
// [[Rcpp::export]]
// Gibbs-EM training: 
Rcpp::List gibbs_EM( int K, int S, int S1, int D, 
                     double a00, double a01, double a10, double a11, 
                     double b00, double b01, double b10, double b11,
                     Rcpp::NumericVector alpha, 
                     Rcpp::NumericVector rho, 
                     ...
                     Rcpp::NumericMatrix tree_str,
                     int burn_in, 
                     int opt_N_1, int cycle_1, 
                     int opt_N_2, int cycle_2 ) {
  ...
  Rcpp::List Z_sum_2( opt_N_2 );    // 2nd training stage: multiple posterior sample of Z collected in the E-step
  Rcpp::NumericMatrix Z_sum_m( D,K );   // Summary of current topic assignments
  
  // Get the summary of topic assignments:
  Z_sum_m = gibbs_Z_count_parallel( Z, alpha.size(), S1 ); 
  
  // Marginal likelihood: 
  Rcpp::NumericVector L_data( cycle_1 + cycle_2 );
  Rcpp::NumericVector Z_prior( cycle_1 + cycle_2 );
  Rcpp::NumericVector I_prior( cycle_1 + cycle_2 );
  ...
  result_final[1] = I;             
  result_final[2] = rho;      
  result_final[3] = alpha;         
  result_final[4] = Z;
  
  //result_final[5] = L_data;            
  //result_final[6] = Z_prior;             
  //result_final[7] = I_prior;      
  //result_final[8] = Phi_prior;         
  //result_final[9] = rho_prior;
