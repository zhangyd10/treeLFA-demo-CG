...
recorded. Each row in the input matrix is an individual, and each column is a disease code. The example dataset
contains the diagnostic record of 15 disease codes for 5,000 individuals. 
...
Dirichlet prior for topic weights) while doing the inference of other hidden variables. The number of topics to
be inferred (argument "topic.number" in the "gibbs_EM_train" function below) needs to be set. The initial alpha
(argument "alpha") is set to be (0.1,0.1,0.1,0.1), which was the true alpha used to simulate the data. On real 
...
number of hidden variables collected in the E-step can be set (arguement "opt_N_1" and "opt_N_2"). Note that in
stage 1, "opt_N_1" is usually set to be 1 to speed up the traniing. In stage 2, "opt_N_2" can be set to a value
larger than 1 to achieve more accurate optimization of alpha. The Gibbs-EM training here will take about two 
minutes. 
```{r, echo=TRUE}
