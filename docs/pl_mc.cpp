...
struct L_mca : public RcppParallel::Worker { 
  ...
  L_mca( Rcpp::NumericVector alpha, 
         ...
         double IS,
         uint64_t seed ) : alpha(alpha), Phi(Phi), LDA_data(LDA_data), IS(IS), seed(seed), L(0) {}
  
  // Splitting constructor: 
  L_mca( const L_mca& l_mca, RcppParallel::Split ):  
    alpha(l_mca.alpha), Phi(l_mca.Phi), LDA_data(l_mca.LDA_data), IS(l_mca.IS), seed(l_mca.seed), L(0) {}
  ...
  void operator() ( std::size_t begin, std::size_t end ) { 
    ...
    for ( std::size_t d=begin; d<end; d++ ) {     // loop through individuals
      ...
      for ( std::size_t is=0; is<IS; is++ ){    // loop through samples of topic weights (theta)
        ...
        // Calculate predictive likelihood for each individual: disease by disease
...
double pred_L_mca ( Rcpp::NumericVector alpha, 
  ...
  // Create the worker:
  L_mca l_mca( alpha, Phi, LDA_data, IS, seed );
